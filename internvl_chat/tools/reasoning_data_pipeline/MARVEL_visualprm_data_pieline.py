...
class MARVELDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        data,
        sample_max_num=None,
...
def evaluate_chat_model():
    ...
    if os.path.exists(results_file):
        ...
        for line in lines:
            ...
            item2num[(str(item['image_path']), item['question_orig'])] += 1
    ...
    for idx, (inputs, items) in enumerate(dataloader):
        ...
        for i in range(len(inputs)):
            cnt = args.num_return_sequences - item2num[(str(items[i]['image_path']), items[i]['question'])]
            if cnt <= 0:
                continue
        ...
        if len(inputs) <= 0:
            print(
                ...
                f'[Progress {idx+1}/{len(dataloader)}] '
                f'skip'
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prompt-path', type=str, default='/home/ncs/ob1/InternVL/internvl_chat/datasets/MARVEL_AVR_flattened_full.jsonl')
    ...
    parser.add_argument('--prompt-version', type=str, default="en_v2", choices=['en', 'en_v2', 'zh', 'zh_v2', 'en_r1', 'zh_r1'])
