...
# Usage: AZURE_API_KEY='your-key' ./run_test_batch_processor.sh [split_name] [azure_endpoint]
...
AZURE_ENDPOINT=${2:-"https://aisg-sj.openai.azure.com/"}

# Check API key
if [[ -z "${AZURE_API_KEY}" ]]; then
    echo "Error: Set AZURE_API_KEY environment variable"
    echo "Usage: AZURE_API_KEY='your-key' ./run_test_batch_processor.sh [split_name] [azure_endpoint]"
    exit 1
fi
...
screen -S "${SCREEN_SESSION}" -dm bash -c "
    ...
    python test_batch_processor.py --split='${SPLIT}' --azure-endpoint='${AZURE_ENDPOINT}'
