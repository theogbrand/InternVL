...
OUTPUT_DIR="$SCRIPT_DIR/infovqa_open_answer_rollouts_output"
LOG_DIR="$OUTPUT_DIR/screen_logs"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
SCREEN_NAME="infovqa_open_answer_rollout_$TIMESTAMP"
...
start_rollout() {
    echo "Starting InfoVQA Open Answer rollout with streaming support..."
...
status() {
    echo "Active InfoVQA Open Answer rollout sessions:"
    screen -list | grep infovqa_open_answer_rollout || echo "No active sessions"
    echo ""
    echo "Recent logs:"
...
cleanup() {
    echo "Cleaning up..."
    # Kill all infovqa open answer rollout sessions
    screen -list | grep infovqa_open_answer_rollout | cut -d. -f1 | awk '{print $1}' | xargs -I {} screen -S {} -X quit 2>/dev/null
